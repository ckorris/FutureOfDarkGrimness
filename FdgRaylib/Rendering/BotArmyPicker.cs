...
/// <list type="number">
///   <item>Prefer armies CLOSE to the lobby's points limit, closest first; an army over the limit sorts
///   behind every legal one, because the #153 launch gate flags it.</item>
...
///   <item>Don't show a player the same army twice until it has cycled through all of them.</item>
/// </list>
/// </summary>
public sealed class BotArmyPicker
{
    ...
    public ArmyCatalogEntry? PickNext(Guid slot, int pointsLimit, IReadOnlySet<string> inUseByOthers)
    {
        ...
        ArmyCatalogEntry? pick =
            First(ranked, a => !shown.Contains(a.Key) && !inUseByOthers.Contains(a.Key));

        // Everything unseen is taken by someone else, or this slot has now seen the whole folder. Start
        // the rotation over - still preferring an army nobody else holds.
        if (pick is null)
        {
            shown.Clear();
            pick = First(ranked, a => !inUseByOthers.Contains(a.Key))
