...
- **Shaken recovery clears at the force-idle point, not via `ReconcileEndOfActivationStage`.** Clearing the token the moment the unit is force-idled is observably identical to "clears at end of activation" (nothing reads Shaken between ChooseAction and end-of-turn; objectives reconcile at end of *round*), and avoids threading an "entered-shaken" flag down from `ISingleTurnContext` into the disposable `UnitActionContext`. The `!HasMoved && !HasAttacked` guard is what distinguishes "was Shaken at activation start" (recover) from "became Shaken mid-activation via charge/move" (keep for next time) — a unit can only self-inflict Shaken after acting.
