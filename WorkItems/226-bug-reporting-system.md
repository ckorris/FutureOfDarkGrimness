...
## Notes
- 2026-08-08 (post-close addendum, owner-requested): **sending a report now announces it to every player.** The reporter's client posts a Global chat line through the existing relay (`GuiPlayerMessageUI.Submit`) naming what went out - the save clause appears only for a host, mirroring the disclosure. This closes the consent gap raised when reviewing the design: a report carries the OTHER players' chat, and their army lists inside the host's save, and they never see the disclosure the reporter agreed to. Announced after the bundle snapshot (so the notice isn't itself captured in the report), and suppressed when nothing was retained (no local file AND no upload attempted) so it can never claim a report that does not exist. Transparency, not enforcement - a modified client could simply not send it. The disclosure gained a matching line so the reporter knows the table is told. 4 new tests (14 in `BugReportTests`); 1122 app + 2862 engine green, headless smoke exit 0. Item stays closed; this is an amendment, not a reopen.
