...
**Design consequence:** make the catalog format **game-agnostic and source-neutral** so it can hold (a) our own FDG factions we author, or (b) imported OPR books — with the OPR-import path as an isolated, optional adapter. Then "build the tool" and "populate it with OPR data" are independent go/no-go switches. Recommended default: build against **one hand-authored FDG demo book** first; treat OPR import as a later, clearance-gated slice.

---

## Target architecture

New app-side pieces (namespaces illustrative; all under `FdgRaylib/` unless noted):
...
**Where do the catalog types live — app or engine?** **App-side** (decided) to keep the engine submodule read-only. The compiler's *output* (`ArmyListFile`) already lives in the engine's `FDG.SaveLoad`; we only consume it. Revisit only if we later want the engine itself to understand books (not needed for play).
...
6. **Import source:** the OPR **JSON API** (`/api/army-books/<uid>`) returns the exact book, cleanly structured — vastly easier than parsing the two-column PDF tables. Recommend API as the machine source for P0b, PDFs (in `../GDF Armies/`, all 47, V3.5.3) as human ground-truth + offline fallback. *Awaiting user confirm; does not block P0.*
...
- **P0b — OPR import adapter (parallel to P0, format pressure-test).** Map OPR's `/api/army-books/<uid>` JSON → our `.fdgbook` early, so the format is validated against real Army Forge data (e.g. Alien Hives) rather than only our own demo book — the "both together" data-source decision. Isolated adapter; **using/shipping OPR content stays gated on OnePageRules clearance**, but the adapter itself and format-fit can be proven now. *Done-when:* Alien Hives imports to a valid `.fdgbook` and compiles a playable army.
...
2. **Data source:** ✅ Both together — hand-author FDG books AND build the OPR import adapter in parallel (P0 + P0b); OPR *content use* stays gated on OnePageRules clearance.
3. **Screen strategy:** ✅ New `ArmyForgeScreen` alongside the freeform `ArmyBuilderScreen` (kept).
4. **Catalog location:** ✅ App-side (keeps the engine submodule read-only).
