...
**Status**: in-progress (Phase 7 dispatch complete; **Phase 8 integration + the Phase 7h engine-primitive refactor underway. Suite 292/0.** 18 rules live (Thrust on the shared `IsCharging` thread — melee-entry implies charge; Blast on a new hit-multiplier sink, capped at target model count, folded after injection per its "after other rules" ordering; Takedown as a targeting-restructure marker that re-scopes a shooting attack to one chosen model, confining wounds with no carry-over); two engine subsystems now invocable — MOVEMENT (Vanguard) via `MovementExecutor` + the `ExecutableOperation`/`OperationExecutor` imperative-op seam, and DEPLOY (Scout + Ambush) via the defer/reserve primitive. In progress: the five core-COMBAT rules (Thrust + Blast + Takedown done; Impact + Counter next — both need new melee structure). PRECEDENCE settled: operation order is realized by sink-fold/hook placement in the stage pipeline, not a central priority list. Remaining: attack/reactivate primitives (Reactivate/Strafing), morale/casting, and the JSON loader. Live history: `042-implementation-checklist.txt` INTEGRATION PROGRESS cont. 1–17.)
