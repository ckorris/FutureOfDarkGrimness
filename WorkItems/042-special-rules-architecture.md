...
**Status**: in-progress (Phase 7 dispatch complete; **Phase 8 integration + the Phase 7h engine-primitive refactor underway. Suite 297/0.** 20 rules live — the five core-COMBAT rules are now all done: Thrust (shared `IsCharging` thread — melee-entry implies charge), Blast (hit-multiplier sink capped at target model count + ignores the target's cover; the per-weapon "ignores cover/terrain" flags also ride the movement + ranged-target resolver requests via the shared `SightRuleQueries`, an Indirect-terrain seam left false), Takedown (targeting-restructure marker re-scoping a shooting attack to one chosen model), Impact (a new charge-contact sub-stage `ResolveImpactHitsStage` rolling X dice through the existing save→wound pipeline before normal swings), and Counter (strike-order reversal — `DetermineStrikeOrderStage` swaps the combat roles so the charged unit swings first). Two engine subsystems invocable — MOVEMENT (Vanguard) via `MovementExecutor` + the `ExecutableOperation`/`OperationExecutor` imperative-op seam, and DEPLOY (Scout + Ambush) via the defer/reserve primitive. PRECEDENCE settled: operation order is realized by sink-fold/hook placement in the stage pipeline, not a central priority list. Remaining: attack/reactivate primitives (Reactivate/Strafing), morale/casting, and the JSON loader. Live history: `042-implementation-checklist.txt` INTEGRATION PROGRESS cont. 1–19.)
...
- **Multi-facet rules tested at their headline facet**, with the secondary facets noted in code comments as deferred: Counter (−1 Impact per Counter model), Aircraft (Advance-only + 30" straight-line movement), Artillery (enemies −2 from >9", Hold-only), Flying (move *through* units), Strider-vs-Flying terrain-scope distinction. These are Phase 8 execution concerns. **The canonical at-a-glance list of which deferred facets remain for the rules that are now LIVE is `042-implementation-checklist.txt` Appendix C (the partial-facet ledger) — keep it current there rather than re-listing here.**
