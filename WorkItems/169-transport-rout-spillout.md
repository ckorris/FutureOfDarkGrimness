...
## Notes
- 2026-07-08 (cont. 2): **Hand-verify scenario built: `Scenarios/rout-spillout.json`** (+ signal armies `TransportRout-Owner/-Chargers.fdgarmy`). Rhino: Tough(12)/Transport(6)/Q6/D5, pre-wounded 6 (at half strength; a direct kill would need 6 melee wounds - forces the Rout death, not a swing kill). Riders (3 models) start 3" away - embark them mid-game on the first activation (also exercises #035's awaiting-verification embark). AI Berserkers (2x Q2 A2 AP1 melee) 7" out charge the lone on-table target. Realistic dice, seed 42: verified headless end-to-end (`printf "2\n3\n" | ... --headless --scenario` -> embark, AI charge, melee loss, morale fail (needed 6), "Rhino destroyed - 1 unit(s) spill out!", placement, "Riders spills out - Shaken!", Routed; deterministic across reruns, exit 0). GUI checklist: rout banner -> wreck banner -> interactive 6"-zone placement canvas -> Shaken banner -> per-model dangerous-terrain d6 beats.
