...
- **Per-model ACTIVATED abilities** (a joined hero with Caster / Vanguard / Martial Prowess): the activation/offer layer assumes unit-level abilities; needs to offer/resolve a single model's ability inside a host unit. **Caster specifically handled 2026-06-25** (see Notes) — `IsCaster` and the round-start token grant are now model-aware, so a joined caster hero casts via the host unit's pool. Vanguard / Martial Prowess and a *general* per-model activated-ability mechanism remain.
