...
- 2026-06-30: Follow-up (user request) — the unit hover tooltip now also lists the unit's **innate special rules** in a "Special Rules:" section (between Weapons and Tokens): each rule as name + numeric args (e.g. `Tough(3)`) with its description (dimmed). Reads the live `IUnit.RuleDefinitions` (#042 `ResolvedRule`s); engine-internal Disembark/Embark filtered out. App-side only (`TableTooltipOverlay`); build clean, app suite 44/0, headless exit 0. Awaiting GUI hand-verification with the rest of Step 2.
