...
- **2026-06-25** — **Intent tag = an explicit `bool DefaultAnswer` carried on `YesNoRequest`** (renamed from `AiPrefersYes` — see follow-up note), not a semantic enum. The audit asks for "defaults … explicit per question"; a bool *is* the explicit default and is the lightest expression. All 5 production call sites pass `defaultAnswer: true` by name (each is an opt-in to a free/beneficial ability — strike back, Strafing free attack, Ambush deploy, Reactivate, Scout deploy), so behavior is unchanged but now deliberate and reviewable. A future "decline by default" question passes `false`.
- **2026-06-25** — **`DefaultAnswer` is a `[JsonProperty]` private-setter property with a `= true` field initializer, kept OUT of the `[JsonConstructor]`.** Newtonsoft passes a *missing* constructor parameter as the type default (`false`), which would silently flip the default to "no" for any request whose JSON omits the flag — exactly the silent-default footgun the audit targets. As a settable property the field initializer covers the absent-member case (stays `true`), while `[JsonProperty]` forces Newtonsoft to use the private setter so a present member round-trips. A unit test pins all three cases (present-true, present-false, absent→true).
- **2026-06-25 (follow-up)** — **Renamed `AiPrefersYes` → `DefaultAnswer`** at the user's request, to decouple the field from the AI resolver specifically and make it a general per-question default. The rename opens it to any answer-without-a-live-player path; a future request timeout or a pre-selected GUI button could read it next. Pure rename, behavior unchanged. Engine `f3cdaec`.
- **2026-06-25 (follow-up 2)** — **Wired the first additional consumer: CLI `YesNoResolver` EOF** now returns `request.DefaultAnswer` instead of a hardcoded `true`, so piped/automated (EOF) runs honor each question's declared default. App-side only (no engine change); behavior-preserving today since every current default is `true`. 4 tests in `FdgRaylib.Tests/YesNoResolverTests` (EOF→default for both values; explicit y/n still overrides). App suite 29/0, build clean, headless exit 0.
...
- **2026-06-25** — Implemented + verified. Engine suite **767/0**, full `dotnet build` clean (the 2 warnings are pre-existing CS0067 unused-event on the controllers), headless smoke exits 0.
  - `StageResolution/Requests/YesNoRequest.cs`: new `DefaultAnswer` property (see Decisions); convenience ctor gains `bool defaultAnswer = true`.
  - `Ai/Resolvers/AiYesNoResolver.cs`: returns `request.DefaultAnswer`.
  - 5 call sites set `defaultAnswer: true` explicitly: `OfferStrikeBackStage`, `StrafingStage`, `StartOfRoundExtraActionStage`, `DeterminePlayerTurnStage`, `DeployUnitStage`.
