...
- **2026-06-25 (follow-up)** — **Renamed `AiPrefersYes` → `DefaultAnswer`** at the user's request, to decouple the field from the AI resolver specifically and make it a general per-question default. The AI resolver is still the only consumer today, but the rename opens it to other answer-without-a-live-player paths (CLI EOF fallback — currently hardcoded `true` in `YesNoResolver`; a future request timeout; a pre-selected GUI button). Pure rename, behavior unchanged.
