...
**Status**: done
**Related**: engine `19e4b83`, merged to master via engine merge `22ce29e` (branch `069-remove-legacy-special-rules`), #059 (deleted the bulk of the legacy subsystem 2026-06-13), #042 (the live rule system), audit §8 (`Audit-6-10-2026.md`)
...
- 2026-07-04: **Merged to master.** Master had advanced under reconciliation 10 (a parallel session shipped #070 stable save type IDs + Rending per-hit + Counter beat; engine `9b78d5b`, superproject `405d113`) — the "master is synced" premise was false, surfaced before merging per convention. No code conflict (the #070/Rending/Counter files and the #069 deletions/edits are disjoint). Submodule: FF master to `9b78d5b`, merged `19e4b83` -> merge `22ce29e`; combined suite **1124/1124**. Superproject: FF master to `405d113`, merged the 069 branch; resolved the two expected bookkeeping conflicts by hand — `WorkItemsList.md` (Structure: dropped #069 as done + kept reconciliation 10's #070->#160; Done: union of #070/#039/#154 + #069) and the submodule gitlink (-> `22ce29e`). Post-merge full build + headless re-verified.
