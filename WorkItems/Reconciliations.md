...
> locally, unpushed); the push then bounced because a parallel session had meanwhile landed
> **391 = Harassing charger-side move + Shaken gate** (merged on origin/master). Per merged-wins
> precedent the unmerged local item yields: **cross-game coupling 391 -> 392**
> (`WorkItems/392-parallel-bench-crossgame-coupling.md`; detail file and index line renumbered,
> the unpushed commit amended, no source/comment references existed). 392 verified free on
> origin's index + archive.

> **2026-08-09 — reconciliation 56.** Number **367** went stale DURING the session, the reconciliation-54 shape again. A `git fetch` at session start put the high-water mark at 366, so **367 = ability actions carry their rule description as menu subtext** was filed legitimately; while the work was in progress a parallel session pushed **367 = item-granted rated rules stack** (merged and already archived on origin/master, done 2026-08-08 - the Procession Altar Transport(6) fault found on the same unit, from the same screenshot). Per merged-wins precedent the unmerged local item yields: **menu subtext 367 -> 370** (`WorkItems/370-ability-action-descriptions.md`; index entry, the forward reference in #369's detail file, and three engine source comments updated). **368 (shooting weapon copy count) and 369 (hoverable rules in ability descriptions) were free on origin and keep their numbers.** Left as-is per precedent: the two engine commit messages and two superproject commit messages saying "#367" for this work, which predate the renumber (engine `ac2a9bc`/`b1971ca`, superproject `fa20139`/`e46c4b1`). Nothing had been pushed at renumber time.
...
> **2026-08-12 — number 374 filed clean.** `git fetch origin` before filing put the high-water mark at
...
> **2026-09-04 - number 393 filed clean.** `git fetch origin` before filing: `origin/master`'s index high-water mark 391, archive max 392, detail files max 392 on every remote branch; local `tactician-bc` matches. **393 = Random Army silent outside a dist build.** No collision.
