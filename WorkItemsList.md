...
- [~] 059 — JSON rule-definition loader (the #042 "JSON loader" sub-stream, broken out). **Serialization foundation DONE on master** (engine `5154c5a`+`af3f5bd`): the full `SpecialRuleDefinition` tree round-trips through System.Text.Json as `kind`-tagged JSON — six closed sum types decorated, `RuleJson` options + get-only-property-stripping modifier, per-hierarchy tests + a reflection guard against tag drift, and a corpus test round-tripping all ~27 `CoreRuleCatalog` rules. Remaining (on branch `059-json-rule-loader`): file→registry load at `FDGServer`, validate-at-load via `RuleValidator`, then registry-hash consistency + army-creator pieces. STJ chosen for rules; message/save layer stays Newtonsoft (see #058). ([WorkItems/059](WorkItems/059-json-rule-loader.md))
