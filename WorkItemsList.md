...
- [~] 333 — Melee weapon rules read like shooting's: engine sends them structured (`OptionRules`), the buttons underline the names in place with hover text, and a hover-fed details strip replaces the block of prose under every button (#298). Implemented + tested; awaiting GUI hand-verify. ([WorkItems/333](WorkItems/333-melee-rules-match-shooting.md))
