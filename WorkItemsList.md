...
- [ ] 342 — A saved `.fdgarmy` freezes its book's rule definitions, so a rule implemented later (Heavy Impact, Vengeance, 7 more locally) resolves to nothing and loads as "not implemented". Load now gap-fills from the bundled book; the drop message distinguishes an outdated list from a genuinely unimplemented name. ([WorkItems/342](WorkItems/342-army-rule-definitions-rot.md))
