...
- [ ] 027 — **Weapon-scoped special rules** (`IWeapon.cs` TODOs) — depends on 042. **REQUIRED, not optional.** The #042 rule system currently attaches `RuleDefinitions` (ResolvedRule) only at the **unit** level (`UnitData`), so a unit's Blast/Indirect/AP/etc. apply to *all* its weapons — a model with one Blast weapon and one normal weapon can't be represented, and per-weapon UI attribution (#052) can't be accurate. In real GF these are weapon rules. Need: a weapon can carry #042 rule definitions (attach `ResolvedRule`s to `Weapon`, resolved from the weapon's army-list rule names), and the dispatch/sight queries evaluate per-weapon (`SightRuleQueries` already takes a `Weapon` arg as a forward seam — today unused). Note the two parallel systems to reconcile: legacy `ISpecialRule_Weapon` on `Weapon` vs. #042 `RuleDefinitions`. Until this lands, Blast/Indirect cover/LoS-ignore is unit-wide. The old engine had per-weapon rules; the new one must too. **SEQUENCING (decided 2026-06-11): do this BEFORE the JSON rule/army loader and the army creator.** You can't design a stable army/rule serialization schema without first deciding where rules attach (unit vs. weapon), and `.fdgarmy` files + the builder UI are the most expensive things to migrate. Committing them unit-only would force a save-format version bump + UI rework once weapons land. At minimum the data model — weapons carry a special-rules slot — must be settled and the schema made weapon-attachment-capable before the loader/creator ship; the per-weapon runtime dispatch can follow shortly after.
