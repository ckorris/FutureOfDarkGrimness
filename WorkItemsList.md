...
- [~] 390 — The #206 forced-charge obligation only gated Pass: a unit in the standoff band could Shoot instead, and the shot's HasAttacked then unlocked Pass - the owed charge evaporated. Shoot now grays (with reason) whenever the band binds AND a charge is available; Cast/Move/abilities stay. Implemented + tested; awaiting GUI hand-verify. ([WorkItems/390](WorkItems/390-forced-charge-gates-shoot.md))
...
- [ ] 389 — Walled-in rear shooter (WarriorSistersMovedLaterally save) activates on a PHANTOM kill urgency, then slides laterally. Option 1 shipped (Chris's pick): urgency kill term grounded in standing room + the planner's #363/#384 sight gate; save re-measures 1.00->0.19 with the pick flipping front-first. Pool-gate A/B pending; options 2/3 held for re-observation. ([WorkItems/389](WorkItems/389-rear-shooter-phantom-urgency-lateral-slide.md))
...
- [ ] 392 — FdgLab parallel bench: concurrent games couple through shared state - dop-16 outcomes shift 7/16 when GC mode changes the interleaving (dop-1 identical; each config self-reproduces, so same-config A/Bs stay valid). #193-contract violation; find the leak, then adopt Server GC (2.2x measured) + higher --dop at a re-baseline boundary. ([WorkItems/392](WorkItems/392-parallel-bench-crossgame-coupling.md))
...
- [~] 065 — Networking tests: concurrent-send interleaving, wider lobby protocol coverage. (Stale claim corrected 2026-08-08: real-TCP tests DO exist — `FdgHostConnectionLimitTests` (#266) and `ResumeRejoinNetworkTests` (#187) — and the shared multi-client loopback fixture landed via #188.)
