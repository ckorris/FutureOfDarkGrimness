...
- [~] 319 — A single-target "Replace all X" now leaves the copies that specialist swaps below it were bought for (5 pistols, trade 1, then replace-all -> 4 rifles + the 1), and "-es" plural targets match ("Bashes" -> the "Bash" weapon, which was silently free). Multi-target all-swaps (88 corpus pairs) deferred pending the aggregate weapon-model question. Verify: DAO Union Tactical Grunts - take "Replace all Pulse Rifles" then "Replace one Pulse Rifle -> Plasma Rifle"; expect 4 carbines + 1 plasma at 150 pts. ([WorkItems/319](WorkItems/319-all-swap-yields-and-plurals.md))
- [~] 318 — Replace upgrades starved by a later section: a Replace whose targets another section grants (Titan Lords "Replace any Heavy Hammer" above the shield swap that buys the second hammer) now banks the shortfall and settles it after every section applies, so both hammers swap and an OPR import keeps both. Verify: in the Forge, trade a War Errant Mini-Titan's Titan Shield for a Heavy Hammer, then swap BOTH hammers (385 pts, no hammer left). ([WorkItems/318](WorkItems/318-starved-replace-upgrade.md))
