...
- [~] 277 — Networked decision previews: stream a deciding player's live resolver state (ghosts, planned paths) to all players via a payload-agnostic preview channel; engine transport + app publisher/overlay + movement ghost-path payloads done, awaiting two-instance GUI hand-verify; then other movement-family resolvers opt in. ([WorkItems/277](WorkItems/277-networked-decision-previews.md))
