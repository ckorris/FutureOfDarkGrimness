...
- [~] 309 — Networked client rendered late-deployed (Ambush/disembark/spillout) models label-only until they next moved: client-side UnitData instance replacement vs the renderer's captured reference, plus engine flows clearing off-table tokens AFTER positions replicate. Engine reorder + renderer live-resolve in progress. ([WorkItems/309](WorkItems/309-client-invisible-late-deploy-models.md))
