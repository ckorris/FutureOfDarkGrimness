...
- [~] 309 — Networked client rendered late-deployed (Ambush/disembark/spillout) models label-only until they next moved: engine flows now clear off-table tokens BEFORE positions replicate + renderer resolves the owning unit live per frame. Implemented + tested; awaiting networked GUI hand-verify. ([WorkItems/309](WorkItems/309-client-invisible-late-deploy-models.md))
