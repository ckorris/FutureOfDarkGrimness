...
- [~] 264 — Tactician unit behind large impassible terrain rushes sideways/backwards round 1: 8 causes confirmed by 9 red-by-design pins (`TacticianWalledUnitTests`, branch `264-walled-unit-pins`, Category Pending264); fixes next in order 1->2->3, 5/6, 4, 7, 8. ([WorkItems/264](WorkItems/264-tactician-walled-unit-lateral-retreat.md))
...
- [~] 277 — Networked decision previews: stream a deciding player's live resolver state (ghosts, planned paths) to all players via a payload-agnostic preview channel; movement hand-verified live; all placement/movement sources opted in incl. objective+terrain markers (awaiting hand-verify). ([WorkItems/277](WorkItems/277-networked-decision-previews.md))
...
- [ ] 247 — (exploratory) Range/threat overlay UI rethink: opportunity field + threat frontiers + anchor toggle grew ad hoc and read as cryptic; design pass first, sign-off before building. Relates #162/#224/#230. ([WorkItems/247](WorkItems/247-range-overlay-ui.md))
