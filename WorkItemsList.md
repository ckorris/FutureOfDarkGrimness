...
- [ ] 212 — GUI won't let a unit move THROUGH a friendly unit (should be allowed; only ending on one is illegal). The GUI half of #205's engine/AI fix; stop the GUI move resolver treating friendlies as pass-through blockers. ([WorkItems/212](WorkItems/212-gui-move-through-friendlies.md))
- [ ] 213 — Moving through impassible terrain: GUI should mark the ghost red + un-clickable with a red move line (both group and single mode), instead of letting you place but not press Done; also confirm AI resolvers never move through impassible (see #211). ([WorkItems/213](WorkItems/213-impassible-terrain-move-feedback.md))
