...
- [~] 219 — Bundled `.fdgbook` upgrades OPR prices internally imported as free (hidden numbers unrecoverable). 2026-07-22: warn on lists carrying them (ListValidator) + `--import-book` flagged the whole catalog from the live book endpoint (47/47, 3517 options, flag-only diff); hidden prices still can't be shown. Awaiting GUI hand-verify. ([WorkItems/219](WorkItems/219-army-forge-zero-cost-upgrades.md))
