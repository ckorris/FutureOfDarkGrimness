...
- [ ] 387 — Modified weapon range is invisible: a Battle-Rune'd or mark-extended (or Shrouding-shortened) range shows no "+6\"" anywhere - shoot panel target rows/detail, CLI rows, and movement overlay band labels get a delta indicator off a new request-carried `EffectiveRangeInches`. ([WorkItems/387](WorkItems/387-modified-range-indicator.md))
