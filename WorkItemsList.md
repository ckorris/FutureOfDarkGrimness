...
- [~] 322 — Dice rolls linger without blocking: every `DiceRolledBeat` is held (pacing drops from ~1800ms to a ~600ms settle), panels STACK oldest-anchored/newest-on-top instead of evicting each other, hover freezes them, and attacks animate on a concurrent list. Implemented + tested; awaiting GUI hand-verify (checklist in detail file). ([WorkItems/322](WorkItems/322-dice-stack-non-blocking.md))
