...
- [~] 197 — Faction rule coverage pt.2: dead rule names needing new engine primitives. Corpus dead refs 2,342 -> 2 (1 name, no scope mismatches left); everything filed is shipped except the last row, including the five deferred sub-arms and Vengeance (2026-07-30). Open: Regenerative Strength only. ([WorkItems/197](WorkItems/197-faction-rule-engine-primitives.md))

All 33 GF v3.5.1 core special rules are implemented (verified 2026-06-30; see archive #029-#032/#051).
Corpus coverage is a different story: 2 of 13,870 book rule references (0.01%) do nothing today — see #197 (#196, the data-only half, closed 2026-07-22). The 2026-07-28 passes shipped P22 (all 5 Ambush variants, 199 -> 157), P17 (all 4 place/restore rules, 157 -> 133) Dash (133 -> 127), the Mischievous/Scrapper Boost reroll threshold (127 -> 121), moraleTestThen outside casting (121 -> 114), Strafing (114 -> 102, closing the last scope mismatch) P8's terrain debuffs (102 -> 88) P20's action-permission pair (88 -> 76) P7's No Retreat family (76 -> 67) and Inquisitorial Agent (67 -> 47). 2026-07-29 also shipped Hazardous's self-wound arm (15 refs; no dead-count delta - the name already resolved - but it was the last live BALANCE bug, an AP(4) weapon with its downside missing) and Armor(X)'s defense set (11 refs, also no dead-count delta - #196's marker resolved the name while paid armor changed nothing; now a literal stat SET on Tough's creation seam), plus Unpredictable Marks (5 refs, 47 -> 42: the branch resolver now scans the DEFENDER's marks), Extended Buff Range (9 refs, 42 -> 33: the HDF radios, a Spell-Conduit-style relay leg in AbilityTargeting via the new EnableBuffRelay capability) and Sergeant (12 refs, 33 -> 21: champion weapon-marking in ListCompiler, owner-chosen over true per-model rules); 2026-07-30 shipped P16's one-shot extra attack (Takedown Strike/Shot, 7 refs, 21 -> 14: a new Combat_OnAttackWindow ability hook, and melee Takedown turned on). 2026-07-30 also shipped P19's out-of-order activation (Coordinate, 3 refs, 14 -> 11: a rule-agnostic turn-order primitive, ally-targetable with the owner keeping control) and Surprise Attack (2 refs, 11 -> 9: a mandatory first-activation hit pool - fractional, unlike Storm's decisive one - plus Infiltrate's deployment arm), then Instinctive (4 refs, 9 -> 5: the compelled-attack primitive - the obligation is decided when the unit ACTIVATES and stamped as a token, so a unit that walks into range attacks normally, while a compelled one has its menu collapsed to the attack and both choosers narrowed to the closest valid target; a first cut that also compelled a move-to-attack was cut on the owner's 2026-07-31 rules research). 2026-07-30 also closed the **deferred sub-arms** — partial mechanics on names that already resolved, so no dead-count delta: Ravage now rolls per swinging unit (its strike-back arm, plus the Counter-swap case it exposed), Reinforcement fires on the transport-spillout Shaken, and Mobile Artillery's defensive arm shipped on a new round-scoped MovedThisRound token. The fourth, "Speed Feat Buff", was struck: no such rule exists anywhere in the corpus or history. 2026-07-30 also shipped Vengeance (3 refs, 5 -> 2: markers on the unit that DESTROYED the bearer - a new IHasKillerUnit capability plus Effect.GrantTokenToKiller, since a passive rule's tokens could only ever land on its own bearer; the read side was P14b's persistent marker unchanged). Two filed premises were struck with it: the blocker was not a missing magnitude source (a carrier count resolves to 0 at that hook, and a game-start variant would have credited every model of the unit), and #100's census had the mechanic inverted. Still open: Regenerative Strength only. ([WorkItems/197](WorkItems/197-faction-rule-engine-primitives.md))
