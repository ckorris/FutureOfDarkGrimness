...
- [~] 315 — Limited weapons were mandatory: the shoot loop re-offered every weapon with no exit once one had fired, so a once-per-game weapon got burned against the player's will. Per-weapon "Hold fire" + "Done shooting" (confirmed) + ONCE PER GAME / SPENT badges in both front ends. Implemented + tested + CLI hand-verified; awaiting GUI hand-verify. ([WorkItems/315](WorkItems/315-limited-hold-fire.md))
- [~] 318 — Melee hold-back narrowed to Limited weapons only: "models within 2in/4in of enemies must strike with all melee weapons", so an ordinary weapon gets no opt-out at all; Limited's once-per-game clause is the one implied exception. Implemented + tested + CLI hand-verified; awaiting GUI hand-verify. ([WorkItems/318](WorkItems/318-melee-hold-back-limited-only.md))
- [~] 317 — Companion actions on a menu row: `StringSelectionRequest.SecondaryActions` pairs an option with a second action (melee's "Hold back"), drawn as a button to the RIGHT of its row sharing the row's letter under Shift, instead of a peer list entry. Implemented + tested + CLI hand-verified; awaiting GUI hand-verify. ([WorkItems/317](WorkItems/317-menu-companion-actions.md))
- [~] 316 — Limited was not enforced in melee at all (usable every round forever, undeclinable). Spent-gating + per-model MarkFired over the in-range swingers, plus "Hold back" rows and a confirm on the hold-back that ends the attack; at least one weapon must still swing after charging. Implemented + tested + CLI hand-verified; awaiting GUI hand-verify. ([WorkItems/316](WorkItems/316-melee-limited-not-enforced.md))
