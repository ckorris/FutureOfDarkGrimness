...
- [~] 219 — Bundled `.fdgbook` upgrades imported as free. 2026-07-22: premise overturned - prices ARE published in a per-unit `costs[]` array; importer now reads them (engine) + `--import-book` re-priced the catalog (47/47, 3467 real prices, HEF Noble matches Army Forge). Residual: verbatim share-import still parks upgrades in UnattributedPoints (total correct; #241 follow-up). Awaiting GUI hand-verify. ([WorkItems/219](WorkItems/219-army-forge-zero-cost-upgrades.md))
