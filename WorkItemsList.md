...
- [~] 325 — Dice panels outlive their beats and stack (oldest anchored at the bottom, newest on top, dimmed by depth, hover to freeze) instead of the next roll evicting the last; attacks animate on a concurrent list. Roll pacing deliberately unchanged. Implemented + tested; awaiting GUI hand-verify (checklist in detail file). Was #322 pre-reconciliation-43. ([WorkItems/325](WorkItems/325-dice-stack-non-blocking.md))
- [~] 326 — Token reads crashed the renderer mid-draw (`Collection was modified` out of `TokenChipRenderer`): `TokenContainer` handed out its live list / a lazy view, so the render thread enumerated while a rule mutated. Locked reads + snapshots, contract pinned on `ITokenContainer`. Implemented + tested; awaiting GUI hand-verify. ([WorkItems/326](WorkItems/326-token-container-render-thread-race.md))
