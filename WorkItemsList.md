...
- [ ] 183 — **(HIGH PRIORITY — design fork, needs sign-off)** Hero-join Subject-seat rule attribution: hero-carried defensive rules beyond the 3 special-cased (Stealth/Fearless/Regeneration) silently stop firing when the hero joins a unit. Decide: selective relocation in `HeroJoinResolver` vs generalizing per-model dispatch to every Subject-seat site. Audit §8.17; relates #093/#006.
