...
- [~] 340 — A rotation was still applied to the square the model was STANDING on: every leg was swept at its arriving attitude, so a model beside a wall could not "move out then turn" (#150's auto-facing hit it with no keypress too). A leg is now blocked only when both endpoint attitudes collide, each node's pose is checked strictly, and the move beat carries per-waypoint facings so the turn animates. Movement + consolidation; implemented + tested, awaiting GUI hand-verify. ([WorkItems/340](WorkItems/340-rotation-belongs-to-the-node.md))
