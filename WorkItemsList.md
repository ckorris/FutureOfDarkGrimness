...
- [~] 318 — "Waiting on" line in the status HUD: second smaller top-center line listing non-local players' outstanding tasks (dim prefix + player-colored name + task), replacing the removed Outstanding Tasks window; engine exposes `LocalPlayerIDs`. Implemented + tested; awaiting networked GUI hand-verify. ([WorkItems/318](WorkItems/318-waiting-on-hud-line.md))
