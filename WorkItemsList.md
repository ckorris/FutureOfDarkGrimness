...
- [ ] 307 — A load the Forge rejects (any plain, non-Forge `.fdgarmy`) mutates nothing but a greyed-out hint, so the next Save writes the untouched startup default — an empty Alien Hives 1k list — over whatever path the user picks, source file included. ([WorkItems/307](WorkItems/307-forge-failed-load-saves-default.md))
- [ ] 344 — A saved `.fdgarmy` freezes its book's rule definitions, so a rule implemented later (Heavy Impact, Vengeance, 7 more locally) resolves to nothing and loads as "not implemented". Load now gap-fills from the bundled book; the drop message distinguishes an outdated list from a genuinely unimplemented name. ([WorkItems/344](WorkItems/344-army-rule-definitions-rot.md))
- [ ] 345 — Impact-family rules were unreachable on 138 units: every APC/tank/speeder has Impact and no melee weapon, and the charge gate refused a weaponless unit. Charging now asks whether anything fires at `Melee_OnChargeContact` (Actor seat), and a contact-but-unarmed melee resolves through strike-back instead of skipping it. ([WorkItems/345](WorkItems/345-impact-only-charges.md))
