...
- [~] 279 — Lobby network teardown: abandoned host lobby kept the port as a half-alive zombie (host UI blind to joins, one-way chat, ghost roster slots); Dispose is now full teardown + app tears down on back/exit + port-bind failure surfaced in HostModal. Implemented + tested; awaiting GUI hand-verify. ([WorkItems/279](WorkItems/279-lobby-network-teardown.md))
