...
- [~] 265 — Unit-wide abilities were offered on `Condition.Always`, so a joined hero's Teleport teleported the whole squad; Teleport/Vanguard/Fanatic/Martial Prowess + the four supplement reposition rules now gate on `AllModelsHaveThisRule`, enforced by a new `RuleValidator.ValidateAuthoring` (kept out of the load gate so old army files still open). Implemented + tested; awaiting GUI hand-verify. ([WorkItems/265](WorkItems/265-all-models-gate-unit-wide-abilities.md))
