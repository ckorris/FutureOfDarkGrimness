...
- [~] 337 — A Shaken unit looked like any other in the activation picker, so its no-action activation read as the forced-charge rule failing (it isn't: that gate is base-to-base, verified + pinned). Engine appends `(Shaken - recovers)` to the option label (CLI too); GUI draws it amber, underlined, hover-explained. Implemented + tested + headless-verified; awaiting GUI hand-verify. ([WorkItems/337](WorkItems/337-shaken-badge-in-activation-picker.md))
