...
- `WorkItems/Reconciliations.md` — number-collision log. Numbers are permanent and never reused; read the log before filing new numbers on a branch that has drifted from origin/master. A per-clone pre-push hook (`.git/hooks/pre-push`, not version-controlled — install snippet in `WorkItems/README.md`) blocks duplicate numbers across index + archive.
