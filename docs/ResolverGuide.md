...
| `AssignWoundsRequest` | `AssignWoundsResolver` | `GuiAssignWoundsResolver` | Stateful — `AssignWoundsResults` accumulates clicks; auto-completes when full. #401: the request carries a queue of `WoundPacket`s (one plain pool, or one confined clump per Deadly failed save); build the results from `request.Packets`, never from the headline count. Each click commits the NEXT packet to the picked model — a clump lands what fits and loses the rest — so every ordering rule and Deadly's no-carry-over are enforced in the results object, not the resolver. Header/hover wording is shared via `WoundAssignmentText` |
