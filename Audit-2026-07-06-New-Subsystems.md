...
**Cons -> recommendations:**
- **A hero-join rule-attribution gap (high severity — see §8 Bug 1).** `HeroJoinResolver` relocates every non-Hero rule from a joining hero onto the hero model unconditionally, but per-model dispatch only exists at the Actor seat; every Subject-seat call site passes `models: null`. Any hero-carried defensive rule that isn't one of the 3 rules special-cased via `Condition.AllModelsHaveThisRule` (Stealth/Fearless/Regeneration) silently stops firing the moment the hero joins a unit.
...
**Test coverage notes:** Strong for the paths that are wired — weapon-batch composition, hero attribution for the *attacker* side across 4 scenarios, and movement budgets. The highest-risk gap is exactly where the headline bug lives: no test exercises a hero-carried **Subject-seat** rule post-merge.
...
17. **Hero-join Subject-seat rule attribution (§3, high severity).** A hero carrying a plain defensive (Subject-seat) rule — e.g. Evasive — silently stops working the moment it joins a unit, because `HeroJoinResolver` relocates the rule onto the hero model, but per-model dispatch was never extended past the 3 special-cased defensive gates (Stealth/Fearless/Regeneration) to every other Subject-seat call site. **This is a design fork per CLAUDE.md's "surface non-trivial architecture decisions" convention** — the fix is either making `HeroJoinResolver` selective about what it relocates, or generalizing hero-aware dispatch to every Subject-seat site. Needs your sign-off before either path is built.
...
1. Hero-join Subject-seat rule attribution (§8 item 17) — the highest-severity open finding in this audit. Affects any army using a hero with a plain defensive rule.
